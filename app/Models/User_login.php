...
class User_login extends Model
{
    ...
    protected $allowedFields = ['user_id', 'username', 'password', 'last_login_time', 'last_login_ip', 'default_password_change', 'password_change_time', 'is_active', 'created_by', 'created_on', 'updated_by', 'updated_on'];

    public $user_login_id, $user_id, $username, $password, $last_login_time, $new_password, $last_login_ip, $default_password_change, $password_change_time, $is_active, $created_by, $created_on, $updated_by, $updated_on, $db;

    public function __construct(){
        parent::__construct();
        $this->db= \Config\Database::connect('default'); // connect to database.
        helper(['form', 'url']);
        $this->user_login_id = "";
        $this->user_id = 0;
        $this->username = "";
    ...
    function validate_login() {
        ...
        $query = $this->db->query("SELECT user_login_id, ".$this->table.".user_id, name, email_id, mobile_no, username,user_type.user_type, password, default_password_change FROM ".$this->table." INNER JOIN user ON (".$this->table.".user_id=user.user_id) INNER JOIN user_type ON (user.user_type_id=user_type.user_type_id) WHERE username = '$username' AND ".$this->table.".is_active='$is_active'");
        ...
        //             ];
        //             $sql = "UPDATE ".$this->table." SET last_login_time =:last_login_time WHERE user_login_id=:user_login_id";
        //             $stmt= $this->conn->prepare($sql);
        //             $stmt->execute($data);
        //             // print_r($stmt);exit; 
        //             $stmt->closeCursor();
